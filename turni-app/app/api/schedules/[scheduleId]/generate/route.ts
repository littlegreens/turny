...
const generateBodySchema = z.object({
  /** Se true: elimina le assegnazioni automatiche e ricalcola (le manuali restano fisse). */
...
export async function POST(request: Request, { params }: Params) {
  ...
  try {
    ...
    const parsedBody = generateBodySchema.safeParse(raw);
    if (parsedBody.success) regenerate = parsedBody.data.regenerate === true;
  } catch {
  ...
  let removedAuto = 0;
  if (regenerate) {
    const del = await prisma.shiftAssignment.deleteMany({
      where: { scheduleId, isAutoGenerated: true },
    });
    removedAuto = del.count;
  }
  ...
  function includeInSchedulerMonthly(c: { type: unknown; note?: string | null }): boolean {
    ...
    if (t === "CUSTOM") {
      const n = (c.note ?? "").trim();
      return n === "GENERIC_DAY_UNLOCK" || n === "GENERIC_SHIFT_UNLOCK" || n === "EXTRA_AVAILABILITY";
    }
    return false;
  }
  ...
  return NextResponse.json({
    ...
    removedAuto: regenerate ? removedAuto : 0,
    ...
    solverMeta: result.solveMeta ?? null,
