...
export function ScheduleReportCsvButton({ filename, rows }: Props) {
  function download() {
    ...
    if (r0 && isCompactRow(r0)) {
      header = ["Nome", "Turni", "Ore", "Giorni_liberi", "WE_liberi"];
      lines = [
        header.join(","),
        ...(rows as RowCompact[]).map((r) =>
          [r.label, String(r.shiftCount), String(r.hoursTotal), String(r.freeDays), String(r.freeWeekend)]
            .map((c) => csvEscape(String(c)))
            .join(","),
        ),
      ];
    } else {
      header = ["Nome", "Email", "Ruolo professionale", "Turni", "Ore", "Modalita contratto"];
      lines = [
        header.join(","),
        ...(rows as RowFull[]).map((r) =>
          [r.label, r.email, r.professionalRole, String(r.shiftCount), String(r.hoursTotal), r.contractMode]
            .map((c) => csvEscape(String(c)))
            .join(","),
        ),
      ];
