...
import { isShiftActiveOnDate, isFestivoOverride, parseHolidayOverrides, type HolidayOverrideDraft } from "@/lib/holiday-overrides";
...
import { scanRoleCompositionIssues } from "@/lib/role-composition";
import type { ScheduleExtraPerson } from "@/lib/schedule-extra-people";
...
import {
  activeShiftTypesForSlotOnDate,
  layoutSlotsForDate,
  WEEKLY_SLOT_LABELS,
  type WeeklySlot,
} from "@/lib/weekly-unavailability";

type SolverMetaPayload = {
  wallTimeSec?: number;
  cpStatus?: string;
  timeLimitSec?: number;
  passLabel?: string;
...
function solverMetaHint(meta: SolverMetaPayload | null | undefined): string {
  if (meta?.wallTimeSec == null) return "";
  const sec = meta.wallTimeSec;
  const quality =
    meta.cpStatus === "OPTIMAL"
      ? "ottimo provato dal motore"
      : "soluzione ammissibile (può esistere di meglio entro i vincoli)";
  const budget =
    meta.timeLimitSec != null && sec < meta.timeLimitSec * 0.15
      ? " — fermato presto, spazio di ricerca esplorato in parte"
      : "";
  return ` Motore OR-Tools: ${sec}s, ${quality}${budget}.`;
}
...
/** Deriva solo dayOff dalle fasce (dayMust si imposta solo cliccando il giorno). */
function syncMemberDayOffFromShifts(
  date: string,
  shiftOff: Record<string, boolean>,
  shiftMust: Record<string, boolean>,
  shiftTypesList: ShiftTypeCol[],
  isActive: (date: string, st: ShiftTypeCol) => boolean,
): boolean {
  const activeSts = shiftTypesList.filter((st) => isActive(date, st));
  if (!activeSts.length) return false;

  const allUnavailable = activeSts.every((st) => {
    const k = `${date}|${st.id}`;
    return shiftOff[k] && !shiftMust[k];
  });
  return allUnavailable;
}

function dateHasSlotMust(
  date: string,
  shiftOff: Record<string, boolean>,
  shiftMust: Record<string, boolean>,
  shiftTypesList: ShiftTypeCol[],
  ...
  return shiftTypesList.some((st) => {
    if (!isActive(date, st)) return false;
    const k = `${date}|${st.id}`;
    return shiftMust[k] && !shiftOff[k];
  });
}
...
type MemberOpt = {
  ...
  /** Anagrafica: non lavora nei giorni festivo del periodo. */
  noHolidayWork?: boolean;
  /** Coppie ricorrenti «weekday|shiftTypeId» (indisponibilità settimanali HARD). */
  baseBlockedShiftKeys: string[];
  memberColor: string | null;
  /** Colore anagrafica (senza override calendario), per anteprima «usa anagrafica». */
  orgDisplayColor?: string | null;
  /** Solo override calendario (#hex); se null → griglia usa org/resolve. */
  calendarColorOverride: string | null;
  /** Preferenza solver per coprire buchi in questo periodo. */
...
export type GridAssignment = {
  ...
  guestExtraPersonId?: string;
  guestProfessionalRole?: string;
  shiftTypeId: string;
  date: string;
  memberLabel: string;
...
function shiftDateAddIso(dateStr: string, days: number): string {
  const d = new Date(`${dateStr}T00:00:00.000Z`);
  d.setUTCDate(d.getUTCDate() + days);
  return d.toISOString().slice(0, 10);
}

function isNightShiftType(st: Pick<ShiftTypeCol, "name" | "startTime" | "endTime">): boolean {
  return st.name.toLowerCase().includes("notte") || st.endTime <= st.startTime;
}

function memberHadNightOnDate(
  memberId: string,
  dateStr: string,
  ...
  return assignments.some((a) => {
    if (a.isGuest || a.memberId !== memberId || a.date !== dateStr) return false;
    const st = shiftTypes.find((x) => x.id === a.shiftTypeId);
    return st ? isNightShiftType(st) : false;
  });
}
...
type Props = {
  ...
  extraPeople?: ScheduleExtraPerson[];
  ...
  /** Timestamp ultima generazione automatica; assente se mai generato o dopo «Svuota». */
...
function serializeMemberPopupState(
  ...
  const parts: string[] = [
    `vac:${vacationDays}`,
    `extra:${extraAvailability ? 1 : 0}`,
    useOrgColor ? "ORG1" : `HEX:${color}`,
  ];
...
export function ScheduleGridPanel({
  ...
  const festivoDateSet = useMemo(() => {
    const scheduleDates = new Set(parseHolidayOverrides(scheduleRules).map((h) => h.date));
    return new Set(holidayList.filter((h) => isFestivoOverride(h, scheduleDates)).map((h) => h.date));
  }, [holidayList, scheduleRules]);
  const noHolidayWorkMemberIds = useMemo(() => {
    const s = new Set<string>();
    for (const m of members) {
      if (m.noHolidayWork) s.add(m.id);
    }
    return s;
  }, [members]);
  ...
  const [extraRoleDraft, setExtraRoleDraft] = useState("");
  ...
  const [dragExtraPersonId, setDragExtraPersonId] = useState<string | null>(null);
  ...
  const [memberExtraAvailabilityDraft, setMemberExtraAvailabilityDraft] = useState(false);
  ...
  const gridAssignments = useMemo(() => {
    ...
    return assignments.filter((a) => {
      if (a.isGuest) {
        return parseProfessionalRoles(a.guestProfessionalRole ?? "").includes(configRoleFilter);
      }
  ...
  const extraPeopleForSidebar = useMemo(() => {
    if (!configRoleFilter) return extraPeople;
    return extraPeople.filter((p) => parseProfessionalRoles(p.professionalRole).includes(configRoleFilter));
  }, [extraPeople, configRoleFilter]);
  ...
  /** Controlli/alert griglia: solo dopo «Genera turni» e con almeno un'assegnazione (non su griglia vuota). */
  const showGridControls = useMemo(() => {
    if (assignments.length === 0) return false;
    return Boolean(lastAutoGenerateAt) || assignments.some((a) => a.isAutoGenerated === true);
  }, [assignments, lastAutoGenerateAt]);

  const roleCompositionIssues = useMemo(() => {
    if (!showGridControls) return { keys: new Set<string>(), rows: [] as { key: string; dateStr: string; shiftName: string; message: string }[] };
    const membersById = new Map(members.map((m) => [m.id, { id: m.id, professionalRole: m.professionalRole }]));
    return scanRoleCompositionIssues({
      dates,
      shiftTypes,
      isShiftActive: (dateStr, st) => shiftActive(dateStr, st as ShiftTypeCol),
      assignments: assignments.map((a) => ({
        date: a.date,
        shiftTypeId: a.shiftTypeId,
        memberId: a.memberId,
        isGuest: a.isGuest,
        guestProfessionalRole: a.guestProfessionalRole,
      })),
      membersById,
    });
  }, [showGridControls, dates, shiftTypes, assignments, members, holidayList]);

  const roleCompositionDates = useMemo(() => {
    const s = new Set<string>();
    for (const r of roleCompositionIssues.rows) s.add(r.dateStr);
    return s;
  }, [roleCompositionIssues.rows]);

  /** Griglia già compilata dal motore almeno una volta → azione principale = rigenera. */
  const isRegenerateMode = useMemo(() => {
    return Boolean(lastAutoGenerateAt) || assignments.some((a) => a.isAutoGenerated === true);
  }, [assignments, lastAutoGenerateAt]);
  ...
  const staleAssignmentCellKeys = useMemo(() => {
    const keys = new Set<string>();
    for (const a of assignments) {
      const st = shiftTypes.find((s) => s.id === a.shiftTypeId);
      if (st && !shiftActive(a.date, st)) keys.add(`${a.date}|${a.shiftTypeId}`);
    }
    return keys;
  }, [assignments, shiftTypes, holidayList]);
  ...
  const riepilogoAllPeople = useMemo(() => {
    const rows: Array<{
      id: string;
      label: string;
      isExtra: boolean;
      ...
      label: r.label,
      isExtra: false,
      rowColor: memberById.get(r.memberId)?.memberColor ?? null,
      professionalRole: memberById.get(r.memberId)?.professionalRole ?? "",
    ...
    for (const er of reportSummary.extraRows) {
      const worked = new Set(
        assignments
          .filter(
            (a) =>
              a.isGuest &&
              `g:${(a.guestLabel ?? a.memberLabel).trim()}|${a.guestColor ?? ""}` === er.key,
          )
          .map((a) => a.date),
      );
      let freeDays = 0;
      ...
      const ep = extraPeople.find((p) => p.label === er.label && p.color === er.color);
      rows.push({
        id: `e:${er.key}`,
        label: er.label,
        isExtra: true,
        rowColor: er.color,
        professionalRole: ep?.professionalRole ?? "",
        shiftCount: er.shiftCount,
        nightCount: 0,
        satCount: 0,
        festiveCount: 0,
        hoursTotal: er.hoursTotal,
        freeDays,
      });
    }
    ...
    return rows;
  }, [riepilogoPerPersona, reportSummary.extraRows, assignments, dates, memberById, extraPeople]);
  ...
  function extraRolesValid(raw: string): boolean {
    return parseProfessionalRoles(raw).length > 0;
  }
  ...
  function openExtraEditPopup(extraId: string) {
    const p = extraPeople.find((x) => x.id === extraId);
    if (!p || !canEdit) return;
    setEditingExtraId(extraId);
    setCellAddOpen(null);
    setGuestNameDraft(p.label);
    setGuestColorDraft(p.color.match(/^#[0-9A-Fa-f]{6}$/) ? p.color : "#6b7280");
    setExtraRoleDraft(p.professionalRole);
  }

  async function updateExtraPerson() {
    if (!editingExtraId) return;
    const trimmed = guestNameDraft.trim();
    if (!canEdit || !trimmed || !extraRolesValid(extraRoleDraft)) return;

    const normalized = trimmed.replace(/\s+/g, " ").toLowerCase();
    const matchesMember = members.some((m) => (m.label ?? "").trim().replace(/\s+/g, " ").toLowerCase() === normalized);
    if (matchesMember) {
      showToast("error", "Questo nome coincide con una persona in anagrafica.");
      return;
    }
    const dup = extraPeople.some(
      (p) => p.id !== editingExtraId && p.label.replace(/\s+/g, " ").toLowerCase() === normalized,
    );
    if (dup) {
      showToast("error", "Esiste già un extra con questo nome.");
      return;
    }

    const color = guestColorDraft.match(/^#[0-9A-Fa-f]{6}$/) ? guestColorDraft : "#6b7280";
    setLoadingKey(`extra-edit-${editingExtraId}`);
    const res = await fetch(`/api/schedules/${scheduleId}/extra-people`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        id: editingExtraId,
        label: trimmed,
        color,
        professionalRole: extraRoleDraft.trim(),
      }),
    });
    ...
    closeExtraPopup();
    showToast("success", "Extra aggiornato.");
    router.refresh();
  }

  async function saveExtraPopup() {
    if (editingExtraId) await updateExtraPerson();
    else await addExtraFromCellPopup();
  }

  async function addExtraFromCellPopup() {
    if (!cellAddOpen) return;
    const trimmed = guestNameDraft.trim();
    if (!canEdit || !trimmed || !extraRolesValid(extraRoleDraft)) return;
    ...
    const matchesMember = members.some((m) => (m.label ?? "").trim().replace(/\s+/g, " ").toLowerCase() === normalized);
    if (matchesMember) {
      showToast("error", "Questa persona esiste già in anagrafica: trascinala dalla lista.");
      return;
    }

    const color = guestColorDraft.match(/^#[0-9A-Fa-f]{6}$/) ? guestColorDraft : "#6b7280";
    const existing = extraPeople.find((p) => p.label.replace(/\s+/g, " ").toLowerCase() === normalized);

    setLoadingKey(`${cellAddOpen.date}|${cellAddOpen.shiftTypeId}|extra`);
    let extraId = existing?.id;
    if (!extraId) {
      const createRes = await fetch(`/api/schedules/${scheduleId}/extra-people`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ label: trimmed, color, professionalRole: extraRoleDraft.trim() }),
      });
      const createPayload = (await createRes.json()) as { error?: string; extraPerson?: { id: string } };
      if (!createRes.ok) {
        setLoadingKey(null);
        showToast("error", createPayload.error ?? "Errore salvataggio extra");
        return;
      }
      extraId = createPayload.extraPerson?.id;
      if (!extraId) {
        setLoadingKey(null);
        showToast("error", "Risposta non valida");
        return;
      }
    }

    const assignRes = await fetch(`/api/schedules/${scheduleId}/assignments`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        date: cellAddOpen.date,
        shiftTypeId: cellAddOpen.shiftTypeId,
    ...
    setLoadingKey(null);
    if (!assignRes.ok) {
      showToast("error", assignPayload.error ?? "Errore salvataggio");
      return;
    }
    closeExtraPopup();
    showToast("success", "Extra aggiunto.");
    router.refresh();
  }

  async function assignExtraPerson(date: string, shiftTypeId: string, extraPersonId: string) {
    if (!canEdit || !extraPersonId) return;
    setLoadingKey(`${date}|${shiftTypeId}|extra`);
    const res = await fetch(`/api/schedules/${scheduleId}/assignments`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ date, shiftTypeId, extraPersonId }),
    });
    const payload = (await res.json()) as { error?: string };
    setLoadingKey(null);
    if (!res.ok) {
    ...
    setCellAddOpen(null);
    showToast("success", "Extra assegnato.");
    router.refresh();
  }
  ...
  async function generateAuto(regenerate: boolean) {
    if (!canEdit) return;
    ...
    try {
      ...
      let payload: {
        ...
        solverAlerts?: SolverAlertItem[];
        solverMeta?: SolverMetaPayload | null;
      } = {};
      try {
      ...
      const removedAuto = payload.removedAuto ?? 0;
      const metaHint = solverMetaHint(payload.solverMeta);
      if (regenerate) {
        if (created === 0 && removedAuto === 0) {
          showToast(
            "success",
            `Rigenerazione completata: nessun turno automatico da sostituire (controlla vincoli o copertura già piena).${metaHint}`,
          );
        } else {
          showToast(
            "success",
            `Rigenerazione completata: ${removedAuto} turni automatici sostituiti, ${created} nuove assegnazioni.${metaHint}`,
          );
        }
      } else if (created === 0) {
        showToast(
          "success",
          `Generazione completata: nessun nuovo turno aggiunto (griglia già coperta o vincoli bloccano nuovi slot).${metaHint}`,
        );
      } else {
        showToast("success", `Generazione completata: ${created} assegnazioni create e salvate.${metaHint}`);
      }
  ...
  function openMemberPopup(memberId: string) {
    ...
    for (const d of days) {
      ...
      const blockedKeys = member?.baseBlockedShiftKeys ?? [];
      const activeSts = shiftTypes.filter((st) => shiftActive(d.dateStr, st));
      const hasBaseDay =
        activeSts.length > 0 && activeSts.every((st) => blockedKeys.includes(`${dow}|${st.id}`));
      ...
      const hasAnyMonthlyReqShift = activeSts.some((st) =>
        monthlyUnavailable.some(
          (u) =>
            u.memberId === memberId &&
            u.date === d.dateStr &&
            u.type === "REQUIRED_SHIFT" &&
            u.shiftTypeId === st.id,
        ),
      );
      const dayUnavail = hasMonthlyReqDay ? false : (hasBaseDay && !hasGenericDayUnlock) || hasMonthlyDay;
      dayOff[d.dateStr] = dayUnavail;
      dayMust[d.dateStr] = hasMonthlyReqDay && !hasAnyMonthlyReqShift;
      ...
      for (const st of shiftTypes) {
        ...
        const hasBaseShift = blockedKeys.includes(`${dow}|${st.id}`);
    ...
    const extraInit = member?.extraAvailability ?? false;
    ...
    setMemberExtraAvailabilityDraft(extraInit);
  ...
  function toggleDay(date: string) {
    ...
    flushSync(() => {
      ...
      const activeSts = shiftTypes.filter((s) => shiftActive(date, s));

      if (nOff) {
        setMemberShiftOff((prev) => {
          const u = { ...prev };
          for (const st of activeSts) u[`${date}|${st.id}`] = true;
          return u;
        });
        setMemberShiftMust((prev) => {
          const u = { ...prev };
          for (const st of activeSts) u[`${date}|${st.id}`] = false;
          return u;
        });
      } else if (nMust && !nOff) {
        /** DEVE giorno: almeno una fascia a scelta (REQUIRED_DATE), fasce neutre senza DEVE singole. */
        setMemberShiftOff((prev) => {
          const u = { ...prev };
          for (const st of activeSts) u[`${date}|${st.id}`] = false;
          return u;
  ...
  function popupShiftTypesForSlot(dateStr: string, slot: WeeklySlot) {
    return activeShiftTypesForSlotOnDate(shiftTypes, slot, dateStr, holidayList, (ds, st) =>
      shiftActive(ds, st as ShiftTypeCol),
    );
  }

  function toggleSlot(date: string, slot: WeeklySlot) {
    const sts = popupShiftTypesForSlot(date, slot);
    if (!sts.length) return;

    const rep = sts[0]!;
    const repKey = `${date}|${rep.id}`;
    const base = memberBaseShiftOff[repKey];
    const off = memberShiftOff[repKey];
    const must = memberShiftMust[repKey];
    let nOff = off;
    let nMust = must;
    ...
    const nextShiftOff = { ...memberShiftOff };
    const nextShiftMust = { ...memberShiftMust };
    for (const st of sts) {
      nextShiftOff[`${date}|${st.id}`] = nOff;
      nextShiftMust[`${date}|${st.id}`] = nMust;
    }
    const nextDayOff = syncMemberDayOffFromShifts(date, nextShiftOff, nextShiftMust, shiftTypes, shiftActive);
    let nextDayMust = memberDayMust[date] ?? false;
    if (nextDayOff) {
      nextDayMust = false;
    } else if (dateHasSlotMust(date, nextShiftOff, nextShiftMust, shiftTypes, shiftActive)) {
      nextDayMust = false;
    } else if (nMust && !nOff) {
      nextDayMust = false;
    }
    ...
    flushSync(() => {
      setMemberShiftOff(nextShiftOff);
      setMemberShiftMust(nextShiftMust);
      setMemberDayOff((prev) => ({ ...prev, [date]: nextDayOff }));
      setMemberDayMust((prev) => ({ ...prev, [date]: nextDayMust }));
    });
  }

  function slotTileClass(dateStr: string, slot: WeeklySlot): string {
    const sts = popupShiftTypesForSlot(dateStr, slot);
    if (!sts.length) return "schedule-member-shift-na";
    const dayBase = memberBaseDayOff[dateStr];
    const dayOff = memberDayOff[dateStr];
    const dayMust = memberDayMust[dateStr];
    const dayGenericUnlocked = dayBase && !dayOff && !dayMust;
    let anyRed = false;
    let anyMust = false;
    let anyBypass = false;
    for (const st of sts) {
      ...
      if (active && !smust) anyRed = true;
      if (!active && smust && (base || dayGenericUnlocked)) return "schedule-member-must-generic-bypass-btn";
      if (!active && smust) anyMust = true;
      if (!active && !smust && (base || dayGenericUnlocked)) anyBypass = true;
    }
    if (anyMust) return "btn-success";
    if (anyRed) return "btn-danger";
    if (anyBypass) return "schedule-member-generic-bypass-btn";
    if (dayMust && !dayOff) return "schedule-member-day-must-slot-btn";
  ...
  function toggleShift(date: string, shiftTypeId: string) {
    ...
    const nextShiftOff = { ...memberShiftOff, [key]: nOff };
    const nextShiftMust = { ...memberShiftMust, [key]: nMust };
    const nextDayOff = syncMemberDayOffFromShifts(date, nextShiftOff, nextShiftMust, shiftTypes, shiftActive);
    let nextDayMust = memberDayMust[date] ?? false;
    if (nextDayOff) {
      nextDayMust = false;
  ...
  async function saveMemberPopup() {
    ...
    const constraintsRes = await fetch(`/api/schedules/${scheduleId}/monthly-constraints`, {
      ...
      body: JSON.stringify({
        ...
        items,
        extraAvailability: memberExtraAvailabilityDraft,
      }),
    });
  ...
  function getAssignmentIssueDetails(a: GridAssignment): { message: string; variant: "error" | "warning" } | null {
    if (!showGridControls) return null;
    const st = shiftTypes.find((x) => x.id === a.shiftTypeId);
    if (st && !shiftActive(a.date, st)) {
      return { message: "turno non attivo in calendario per questa data", variant: "error" };
    }
    if (
      !a.isGuest &&
      memberHadNightOnDate(a.memberId, shiftDateAddIso(a.date, -1), assignments, shiftTypes)
    ) {
      return { message: "riposo obbligatorio dopo turno notturno", variant: "error" };
    }
    const k = assignmentDayPersonKey(a);
    ...
    if (
      !a.isGuest &&
      noHolidayWorkMemberIds.has(a.memberId) &&
      festivoDateSet.has(a.date)
    ) {
      return { message: "assegnato nonostante evita festivi", variant: "error" };
    }
    ...
    const cellKey = `${a.date}|${a.shiftTypeId}`;
    if (roleCompositionIssues.keys.has(cellKey)) {
      return { message: "composizione ruoli incompleta su questa fascia (manca PS o degenza)", variant: "error" };
    }
    return null;
  }
  ...
  return (
    <div>
        ...
        <div className="d-flex flex-wrap align-items-center gap-2">
          ...
          {canEdit ? (
            <button
              ...
              onClick={() => {
                if (isRegenerateMode) setRegenerateConfirmOpen(true);
                else void generateAuto(false);
              }}
              ...
              title={
                shiftTypes.length === 0
                  ...
                  : isRegenerateMode
                    ? "Ricalcola i turni automatici; le assegnazioni inserite a mano restano fisse."
                    : undefined
              }
            >
              {loadingKey === "generate"
        ...
        <div className="col-12 col-md-4 col-xl-2">
          ...
          >
            <section className="card shadow-none">
              ...
              {configRoleFilter && membersForSidebar.length === 0 && extraPeopleForSidebar.length === 0 ? (
              ...
              <div className="d-grid gap-1 mb-1">
                ...
                {extraPeopleForSidebar.map((p) => (
                  <button
                    key={p.id}
                    type="button"
                    className="btn btn-sm text-start w-100 py-2 px-3 d-flex align-items-center gap-2"
                    style={{
                      border: `1px solid ${p.color}`,
                      backgroundColor: editingExtraId === p.id ? `${p.color}22` : "#fff",
                      color: p.color,
                      fontWeight: 600,
                      cursor: canEdit ? "grab" : "default",
                    }}
                    draggable={canEdit}
                    onDragStart={(e) => {
                      setDragExtraPersonId(p.id);
                      setDragMemberId(null);
                      e.dataTransfer.setData("application/x-extra-person-id", p.id);
                      e.dataTransfer.effectAllowed = "copy";
                      setDragging(true);
                      setDragGhost(e, p.label);
                    }}
                    onDragEnd={() => {
                      setDragging(false);
                      setHoverCellKey(null);
                    }}
                    onClick={() => openExtraEditPopup(p.id)}
                  >
                    <span className="badge rounded-pill text-bg-secondary flex-shrink-0" style={{ fontSize: 9 }}>
                      Extra
                    </span>
                    <span className="text-truncate">{p.label}</span>
                  </button>
                ))}
              </div>
            </div>
        ...
        <table className="table table-sm table-bordered align-middle mb-0" style={{ minWidth: 620, tableLayout: "fixed" }}>
          ...
          <tbody>
            {days.map((d) => {
              ...
              const rowGap =
                (showGridControls && understaffedDates.has(dateStr)) ||
                (showGridControls && roleCompositionDates.has(dateStr)) ||
                (showGridControls &&
                  shiftTypes.some(
                    (st) => staleAssignmentCellKeys.has(`${dateStr}|${st.id}`) && (byCellDisplay.get(`${dateStr}|${st.id}`) ?? []).length > 0,
                  ));
              return (
                <tr key={dateStr} className={inactiveDay ? "table-light" : undefined} style={{ height: 60 }}>
                  <th
                    ...
                    title={
                      rowGap
                        ? "Questo giorno ha almeno uno slot sotto il minimo organico o con composizione errata"
                        : undefined
                    }
                  >
                  ...
                  {shiftTypes.map((st) => {
                    ...
                    const staleAssignments = shiftInactive && cell.length > 0;
                    const underStaff =
                      showGridControls &&
                      !shiftInactive &&
                      understaffedCells.keys.has(`${dateStr}|${st.id}`);
                    const roleCompositionBad =
                      showGridControls &&
                      !shiftInactive &&
                      roleCompositionIssues.keys.has(`${dateStr}|${st.id}`);
                    const under =
                      underStaff || roleCompositionBad || (showGridControls && staleAssignments);
                    return (
                      <td
                        key={`${dateStr}-${st.id}`}
                        ...
                        title={
                          staleAssignments
                            ? "Persone assegnate su turno non attivo in calendario"
                            : roleCompositionBad
                              ? "Composizione ruoli errata per questa fascia"
                              : underStaff
                        ...
                        onDrop={(e) => {
                          ...
                          if (droppedMemberId) {
                            void addAssignment(dateStr, st.id, droppedMemberId);
                            return;
                          }
                          const droppedExtraId =
                            e.dataTransfer.getData("application/x-extra-person-id") || dragExtraPersonId;
                          if (droppedExtraId) void assignExtraPerson(dateStr, st.id, droppedExtraId);
                        }}
                      >
                      ...
                      {shiftTypes
                        .filter(
                          (st) =>
                            ...
                            staleAssignmentCellKeys.has(`${dateStr}|${st.id}`),
                        )
                        .map((st) => {
                        const cell = byCellDisplay.get(`${dateStr}|${st.id}`) ?? [];
                        const shiftInactive = !shiftActive(dateStr, st);
          ...
          <div className="table-responsive">
            <table className="table table-hover align-middle mb-0">
              <thead className="table-light">
                <tr>
                  <th scope="col" className="ps-4 py-3 border-0 text-secondary text-uppercase small fw-semibold">
                    Persona
                  </th>
                  <th scope="col" className="text-end py-3 border-0 text-secondary text-uppercase small fw-semibold">
                    Turni
                  </th>
                  <th scope="col" className="text-end py-3 border-0 text-secondary text-uppercase small fw-semibold">
                    Notti
                  ...
                  <th scope="col" className="text-end pe-4 py-3 border-0 text-secondary text-uppercase small fw-semibold">
                    Gg liberi
                  </th>
                </tr>
              </thead>
              <tbody>
                {riepilogoAllPeople.map((r) => {
                  const rowBg = r.rowColor ? `${r.rowColor}14` : undefined;
                  return (
                    <tr key={r.id} style={{ backgroundColor: rowBg }}>
                      <td className="ps-4 py-3">
                        <div className="d-flex align-items-center gap-2 flex-wrap">
                          {r.isExtra ? (
                            <span className="badge rounded-pill text-bg-secondary flex-shrink-0" style={{ fontSize: 10 }}>
                              Extra
                            </span>
                          ) : null}
                          <span className="fw-semibold" style={r.rowColor ? { color: r.rowColor } : undefined}>
                            {r.label}
                          </span>
                        </div>
                        {r.professionalRole ? (
                          <div className="small text-secondary mt-1">{r.professionalRole}</div>
                        ) : null}
                      </td>
                      <td className="text-end">{r.shiftCount}</td>
                      <td className="text-end text-secondary">{r.nightCount > 0 ? r.nightCount : "—"}</td>
                      <td className="text-end text-secondary">{r.satCount > 0 ? r.satCount : "—"}</td>
                      <td className="text-end text-secondary">{r.festiveCount > 0 ? r.festiveCount : "—"}</td>
                      <td className="text-end fw-semibold">{r.hoursTotal}</td>
                      <td className="text-end pe-4">{r.freeDays}</td>
                    </tr>
                  );
                })}
              </tbody>
      ...
      <section className="card mt-3 border-0 shadow-sm">
          ...
          {!showGridControls ? (
            <p className="text-secondary small mb-0">
              Genera i turni (con almeno un&apos;assegnazione) per vedere i controlli di copertura.
            </p>
          ) : understaffedCells.rows.length === 0 &&
            roleCompositionIssues.rows.length === 0 &&
            coverageOverAlerts.length === 0 ? (
            <p className="text-secondary small mb-0">Nessun problema: tutte le celle attive rispettano min, max e composizione ruoli.</p>
          ) : (
            <div className="d-grid gap-3">
              {roleCompositionIssues.rows.length > 0 ? (
                <div className="alert alert-danger border border-danger mb-0" role="status">
                  <div className="fw-semibold">Composizione ruoli errata</div>
                  <p className="small mb-2 text-secondary">
                    Slot con persone assegnate ma mix ruoli diverso da quello configurato (es. 2 PS invece di PS + degenza).
                    Meglio lasciare un buco che un mix sbagliato.
                  </p>
                  <div className="small" style={{ maxHeight: 360, overflowY: "auto" }}>
                    <ul className="mb-0 ps-3">
                      {roleCompositionIssues.rows.map((r) => (
                        <li key={r.key}>
                          <strong>{formatDateIt(r.dateStr)}</strong> · {r.message}
                        </li>
                      ))}
                    </ul>
                  </div>
        ...
        <section className="card">
          <div className="card-body py-3">
            <div>
              ...
              <p className="small text-secondary mb-0">Regole generali di co-presenza o esclusione tra persone.</p>
              {!canEditRules ? (
          ...
          <div className="card-body py-3 d-flex flex-wrap justify-content-end gap-2 align-items-center">
            {canEdit ? (
              <>
                ...
                <button type="button" className="btn btn-danger turny-btn-action" onClick={() => setClearOpen(true)} disabled={loadingKey !== null}>
                  Svuota griglia
                </button>
              ...
              <div className="modal-content turny-modal">
                <div className="modal-header px-4 pt-3 pb-3">
                  <h2 className="modal-title h5 mb-0">
                    {editingExtraId ? "Modifica persona extra" : "Aggiungi persona extra"}
                  </h2>
                  <button
                    type="button"
                    className="btn-close"
                    aria-label="Chiudi"
                    onClick={() => void closeExtraPopup()}
                  />
                </div>
                <div className="modal-body px-4 pb-4 pt-0">
                  {cellAddOpen ? (
                    <div className="rounded-3 border bg-light px-3 py-2 mb-4 text-secondary">
                      <span className="fw-semibold text-body">{formatDateIt(cellAddOpen.date)}</span>
                      <span className="mx-2">·</span>
                      <span className="fw-semibold text-body">
                        {shiftTypes.find((s) => s.id === cellAddOpen.shiftTypeId)?.name ?? "—"}
                      </span>
                    </div>
                  ) : null}
                  <div className="d-grid gap-4">
                    <div className="w-100">
                      <label className="form-label fw-semibold mb-2" htmlFor="extra-guest-name">
                        Nome
                      </label>
                      <input
                        id="extra-guest-name"
                    ...
                    <div className="w-100">
                      <span className="form-label fw-semibold mb-2 d-block">Colore</span>
                      <ColorPalettePicker
                    ...
                    <div className="w-100">
                      <span className="form-label fw-semibold mb-2 d-block">Ruoli</span>
                      <ProfessionalRoleInput
                        value={extraRoleDraft}
                        onChange={setExtraRoleDraft}
                        suggestions={professionalRoleSuggestions}
                        orgSlug={orgSlug}
                        canManageGlobalRoles={canManageSchedule}
                        disabled={loadingKey !== null}
                        name="schedule-extra-professional-role"
                        className="form-control w-100"
                      />
                    </div>
                  </div>
                </div>
                <div className="modal-footer turny-modal-footer-split px-4 pb-3 pt-0">
                  <button
                    type="button"
                    className="btn btn-outline-secondary turny-btn-action"
                    onClick={() => void closeExtraPopup()}
                    disabled={loadingKey !== null}
                  >
                    Annulla
                  ...
                  <button
                    ...
                    onClick={() => void saveExtraPopup()}
                    disabled={loadingKey !== null || !guestNameDraft.trim() || !extraRolesValid(extraRoleDraft)}
                  >
                    {editingExtraId ? "Salva" : "Aggiungi"}
                  </button>
                </div>
              </div>
            </div>
      ...
      <ConfirmModal
        ...
        title="Rigenera turni"
        message="Verranno eliminati tutti i turni creati dal motore e ne verrà calcolato un nuovo piano. Le assegnazioni inserite a mano restano fisse."
        ...
        onConfirm={() => {
          ...
          void generateAuto(true);
        }}
      />
      <ConfirmModal
        open={clearOpen}
        title="Svuota griglia"
        message="Confermi l'eliminazione di tutte le assegnazioni del periodo (automatiche e manuali)?"
        confirmLabel="Svuota"
        cancelLabel="Annulla"
        confirmVariant="danger"
                ...
                <div className="modal-body pb-4">
                  {(() => {
                    ...
                    if (mem?.configMaxSundays != null)
                      cfgParts.push({ label: "Max festivi", value: String(mem.configMaxSundays) });
                  ...
                  <div className="row g-3">
                    <div className="col-12 member-popup-options d-flex flex-column gap-3 pt-4">
                      <div className="member-popup-field-row">
                        <span className="member-popup-field-label">Colore</span>
                        <div className="member-popup-field-control">
                          <ColorPalettePicker
                            ...
                            inheritDisplayColor={
                              selectedMemberId
                                ? memberById.get(selectedMemberId)?.orgDisplayColor ??
                                  memberById.get(selectedMemberId)?.memberColor ??
                                  "#3B8BD4"
                                : "#3B8BD4"
                            }
                            disabled={loadingKey !== null}
                      ...
                      <div className="member-popup-field-row">
                        <label className="member-popup-field-label" htmlFor="member-vacation-days">
                          Giorni di ferie
                        </label>
                        <input
                          id="member-vacation-days"
                          type="number"
                          className="form-control form-control-sm member-popup-field-control"
                          style={{ width: "7rem" }}
                          min={0}
                          max={366}
                          step={1}
                      ...
                      <div className="member-popup-field-row">
                        <span className="member-popup-field-label">Extra disponibilità</span>
                        <button
                          type="button"
                          role="switch"
                          aria-checked={memberExtraAvailabilityDraft}
                          aria-label="Extra disponibilità"
                          className={`turny-toggle-switch member-popup-field-control ${memberExtraAvailabilityDraft ? "is-on" : ""}`}
                          onClick={() => setMemberExtraAvailabilityDraft((v) => !v)}
                          disabled={loadingKey !== null}
                        >
                    ...
                    <div className="col-12">
                      <p className="small text-secondary mb-2 lh-sm">
                        Verde contorno = disponibile · Verde pieno = obbligatorio · Rosso pieno = indisponibile
                        <br />
                        Giorno verde pieno + fasce grigio chiaro = DEVE almeno una fascia a scelta · Fascia verde pieno = DEVE quella fascia
                        <br />
                        Bordo rosso = disponibile nonostante vincolo settimanale · Bordo rosso e fondo verde = obbligo nonostante vincolo settimanale
                        <br />
                        Grigio = turno non previsto quel giorno
                      </p>
                      <div className="d-grid gap-2">
                        {days.map((d) => {
                          ...
                          const hasCalendarShifts = shiftTypes.some((st) => shiftActive(d.dateStr, st));
                          const { slots: daySlots, sundayLayout } = layoutSlotsForDate(
                            d.dateStr,
                            holidayList,
                            shiftTypes,
                            (ds, st) => shiftActive(ds, st as ShiftTypeCol),
                          );
                          let offSlots = 0;
                          let activeSlots = 0;
                          for (const slot of daySlots) {
                            const sts = popupShiftTypesForSlot(d.dateStr, slot);
                            if (!sts.length) continue;
                            activeSlots += 1;
                            const slotOff = sts.every((st) => {
                              const k = `${d.dateStr}|${st.id}`;
                              return memberShiftOff[k] && !memberShiftMust[k];
                            });
                            if (slotOff) offSlots += 1;
                          }
                          const allSlotsOff =
                            hasCalendarShifts && activeSlots > 0 && offSlots === activeSlots;
                          let dayTileClass = "btn-outline-success";
                          if (!hasCalendarShifts) dayTileClass = "schedule-member-shift-na";
                          else if (allSlotsOff || (dayOff && !dayMust)) dayTileClass = "btn-danger";
                          else if (!dayOff && dayMust && dayBase) dayTileClass = "schedule-member-must-generic-bypass-btn";
                          else if (!dayOff && dayMust) dayTileClass = "btn-success";
                          else if (!dayOff && !dayMust && dayBase) dayTileClass = "schedule-member-generic-bypass-btn";
                          return (
                            <div key={d.dateStr} className="d-flex align-items-stretch gap-2">
                              <button
                                type="button"
                                className={`btn btn-sm schedule-member-popup-tile ${dayTileClass}`}
                                onClick={() => toggleDay(d.dateStr)}
                                disabled={loadingKey !== null || !hasCalendarShifts}
                                ...
                                title={
                                  dayBase
                                    ? dayOff
                                      ...
                                      : dayMust
                                        ? "DEVE almeno una fascia a scelta"
                              ...
                              <div className="weekly-unavail-slots-grid">
                                {sundayLayout
                                  ? daySlots.map((slot) => {
                                      const wide = slot === "turnoUnico";
                                      const slotInactive = popupShiftTypesForSlot(d.dateStr, slot).length === 0;
                                      return (
                                        <button
                                          key={`${d.dateStr}|${slot}`}
                                          type="button"
                                          className={`btn btn-sm schedule-member-popup-tile ${wide ? "weekly-unavail-slot--wide " : ""}${slotTileClass(d.dateStr, slot)}`}
                                          onClick={() => toggleSlot(d.dateStr, slot)}
                                          disabled={loadingKey !== null || slotInactive}
                                        >
                                  ...
                                  : daySlots.map((slot) => {
                                      ...
                                      return (
                                        <button
                                          key={`${d.dateStr}|${slot}`}
                                          type="button"
                                          className={`btn btn-sm schedule-member-popup-tile ${slotTileClass(d.dateStr, slot)}`}
                                          onClick={() => toggleSlot(d.dateStr, slot)}
                                          disabled={loadingKey !== null || slotInactive}
                                        >
                                          {WEEKLY_SLOT_LABELS[slot]}
                  ...
                  {viewMode === "standard" ? (
                    <div className="w-100 overflow-x-auto" style={{ zoom: previewZoom }}>
                      <ScheduleReadonlyStandardTable
                        ...
                        rowUndersupplied={
                          showGridControls
                            ? (dateStr) => understaffedDates.has(dateStr) || roleCompositionDates.has(dateStr)
                            : undefined
                        }
                        cellUndersupplied={
                          showGridControls
                            ? (dateStr, shiftTypeId) =>
                                understaffedCells.keys.has(`${dateStr}|${shiftTypeId}`) ||
                                roleCompositionIssues.keys.has(`${dateStr}|${shiftTypeId}`)
                            : undefined
                        }
